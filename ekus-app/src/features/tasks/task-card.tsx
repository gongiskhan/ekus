...
export function TaskCard({
  ...
  const handleDragEnd = (_: unknown, info: { offset: { x: number } }) => {
    if (info.offset.x > 80 && onMoveRight) {
      onMoveRight();
    } else if (info.offset.x < -80 && onMoveLeft) {
      onMoveLeft();
    }
  };
    ...
    >
      ...
      <div className="flex items-center gap-1 mt-2 pt-2 border-t" style={{ borderColor: 'var(--border)' }}>
        {onMoveLeft && (
          <button
            onClick={onMoveLeft}
            className="p-1.5 rounded-lg hover:bg-white/40 transition-colors min-w-[32px] min-h-[32px] flex items-center justify-center"
            title="Move left"
          >
